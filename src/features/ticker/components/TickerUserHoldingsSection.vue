...
<template>
  <div v-if="holding || (transactions && transactions.length > 0)" class="space-y-4 font-mono">
    ...
    <div v-if="transactions && transactions.length > 0" class="space-y-2.5">
      ...
      <div class="sm:hidden flex overflow-x-auto snap-x snap-mandatory gap-3 pb-2 scrollbar-none -mx-1 px-1">
        <div
          ...
          :key="'m-' + tx.id"
          class="snap-start shrink-0 w-[72vw] bg-terminal-surface border border-white/10 rounded-2xl p-4 space-y-3 shadow-lg"
        ...
        >
          ...
          <div class="pt-2 border-t border-white/10 flex items-center justify-end">
            ...
            <div v-else-if="pendingDeleteId === tx.id" class="flex items-center gap-1.5">
              <button type="button" class="px-2.5 py-1 rounded-lg text-[11px] font-bold bg-terminal-down/20 text-terminal-down border border-terminal-down/40 hover:bg-terminal-down/35 transition-colors" @click="confirmDelete(tx)">Delete</button>
              <button type="button" class="px-2.5 py-1 rounded-lg text-[11px] font-bold bg-white/5 text-gray-400 border border-white/10 hover:bg-white/10 transition-colors" @click="cancelDelete">Cancel</button>
            </div>
            <button v-else type="button" class="text-gray-500 hover:text-terminal-down transition-colors text-xs px-2 py-1 rounded-lg hover:bg-terminal-down/10" title="Delete transaction" @click="requestDelete(tx.id)">🗑 Remove</button>
          </div>
        </div>
      </div>
      ...
      <div class="hidden sm:grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        <div
          v-for="tx in transactions"
          :key="'d-' + tx.id"
          class="bg-terminal-surface border border-white/10 rounded-2xl p-4 space-y-3 shadow-lg"
          :class="{ 'opacity-40': deletingIds.has(tx.id) }"
        >
          <div class="flex items-center justify-between">
            <span
    ...
    <div v-if="hasPosition" class="space-y-2.5">
      ...
      <div v-else-if="myProjection" class="sm:hidden flex overflow-x-auto snap-x snap-mandatory gap-3 pb-2 scrollbar-none -mx-1 px-1">
        <div
          v-for="r in myProjection.rows"
          :key="'m-' + r.year"
          class="snap-start shrink-0 w-[52vw] bg-terminal-surface border border-white/10 rounded-2xl p-4 space-y-2.5 shadow-lg"
        >
      ...
      <div v-if="myProjection && !projLoading" class="hidden sm:grid grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
        <div
          v-for="r in myProjection.rows"
          :key="'d-' + r.year"
          class="bg-terminal-surface border border-white/10 rounded-2xl p-4 space-y-2.5 shadow-lg"
        >
          <div class="text-sm font-black text-gray-100 border-b border-white/10 pb-2">Year {{ r.year }}</div>
          <div class="space-y-1">
            <div class="text-[10px] text-gray-500 uppercase tracking-wider">Projected Value</div>
